...
public class NavNodeEditor : MonoBehaviour
{
	...
	private void OnDrawGizmos()
	{
		...
		{
			Gizmos.DrawIcon(navNode.transform.position + nodeCameraDirection, "target.png", true, Color.green);
			Gizmos.color = Color.green;
			Gizmos.DrawWireSphere(navNode.gameObject.transform.position, navNode.radius);
		}
		if (activeNavNode != null)
		{
			Gizmos.DrawIcon(activeNavNode.transform.position + activeCameraDirection, "target.png", true, Color.red);
